...
def instantiate_unconditioned_models(device):
        ...
        for subject_id in self.subject_ids:
            ...
            for modality in self.modalities:
                ...
                for resolution in self.resolutions:
                    ...
                    _instance["latent_segmentation_path"] = row["latent_seg_mask"]#self.create_latent_paths(row)
...
class PrepareDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # load latent segmentation
        l_seg = np.load(instance[self.modalities[0]][self.resolutions[0]]["latent_segmentation_path"])
        l_seg = np.where(l_seg > 0, 1.0, 0.0)
        l_seg = np.expand_dims(l_seg, axis=0) # add channel dimension
        example["latent_segmentation"] = l_seg
...
def collate_fn(examples, modalities, resolutions):
    ...
    res_dict["latent_segmentation"] = torch.stack([torch.tensor(example["latent_segmentation"]) for example in examples])
...
@torch.no_grad()
def validation(unet, autoencoder, val_dataloader, step, args):
    ...
    os.makedirs(imgs_2D_output_path, exist_ok=True)
    os.makedirs(metrics_output_path, exist_ok=True)

    for batch in tqdm(val_dataloader, desc="Validation"):

        modality = args_train.modalities[0]

        y_img_path = batch["meta_data"][0]["img_path"][f'{modality}_{args_train.target_resolution}']
        y_img, _ = nfc.load_nifti(y_img_path)
        ...
        y = batch[f'{modality}_{args_train.target_resolution}'].to(device)

        resolution_one_hot = torch.zeros((x.shape[0], len(args_train.x_valid_resolutions)), device=device)
        ...
        with torch.amp.autocast("cuda"):
            ...
            # pred_imgs = autoencoder.decode(predictions)

        # pred_imgs = pred_imgs.clamp(0, 1).cpu().numpy()
        ...
        micro_bs = 1  # or 2 depending on GPU

        for i in range(0, predictions.shape[0], micro_bs):
            with torch.amp.autocast("cuda"):
                pred_i = autoencoder.decode(predictions[i:i+micro_bs])

            pred_imgs_list.append(pred_i.cpu())

        pred_imgs = torch.cat(pred_imgs_list, dim=0)
        pred_imgs = pred_imgs.clamp(0, 1).numpy().squeeze() # remove channel dimension

        # print(f"pred_imgs shape: {pred_imgs.shape}, y shape: {y.shape}")



        for i, res in enumerate(args_train.x_valid_resolutions):
            ...
            metrics_list.append({
                ...
                "modality": modality,
                "resolution": res,
                "mse": util.compute_mse(gt_img, pred_img),
                "mae": util.compute_mae(gt_img, pred_img),
                "ssim": util.compute_ssim(gt_img, pred_img),
            })
            # save images
            # save 2D images 
...
def train(
    ...
    for epoch in range(first_epoch, max_epochs):
        for batch in train_dataloader:
            ...
            latent_masks = batch["latent_segmentation"].to(device).float()  # (batch_size, 1, D, H, W)
            ...
            with autocast("cuda", enabled=args_train.amp):
                ...
                loss_charbonnier = loss_charbonnier_pt(missing_modality_pred.float(), y.float(), mask=latent_masks)  # Dividir para escalar la pérdida
            ...
            if gradient_accumulation_count % args_train.gradient_accumulation_steps == 0:
                ...
                if args_train.initial_val or global_step % args_train.val_interval == 0:
                    ...
                    validation(unet, autoencoder, val_dataloader, global_step, args_train)
...
args_train = {
    # directories 
    "output_path": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test2_direct_dummy_conversion/training/models/test1_masking_loss_faster_val",
...
# /home/agustin/phd/miccai/miccai_2026/mri_x_fields/data/train_data/preprocessed/maisi_latents/T1W_0.1_001_GX_latent.npy
# /home/agustin/phd/miccai/miccai_2026/mri_x_fields/data/train_data/preprocessed/maisi_latents/T1W/0.1T/T1W_0.1T_001_GX_latent.npy
# /home/agustin/phd/miccai/miccai_2026/mri_x_fields/data/train_data/preprocessed/maisi_latents/T1W/0.1/T1W_0.1_001_GX_latent.npy