...
class LoadPaths:
    def __init__(
        self, dataset_path_name, modalities, resolutions, dataset_filters=None
    ):
        ...
        # self.df = self.complete_dataset.df.sort_values(by=['sid', 'age'])
    ...
    #     latent_name = f"{modality}_{resolution}T_{sid[1:]}_latent.npy"
    ...
    def get_data(self, split="train"):
        ...
        # unique_ids = complete_df["sid"].unique()
        ...
        for sid in self.sids:
            ...
            for modality in self.modalities:
                instances[sid][modality] = {}
                for resolution in self.resolutions:

                    row = complete_df[
                        (complete_df["sid"] == sid)
                        & (complete_df["modality"] == modality)
                        & (complete_df["resolution"] == resolution)
                    ].iloc[0]
                    _instance = {}
                    _instance["img_path"] = row["org_img_path"]
                    _instance["latent_path"] = row[
                        "latent_path"
                    ]  # self.create_latent_paths(row)

                    instances[sid][modality][resolution] = _instance
...
class PrepareDataset(Dataset):
    ...
    def load_subject_data(self, instance):
        ...
        for modality in self.modalities:
            for resolution in self.resolutions:
                ...
                subject_meta_data["latent_path"][
                    f"{modality}_{resolution}"
                ] = latent_path
                subject_meta_data["img_path"][f"{modality}_{resolution}"] = instance[
                    modality
                ][resolution]["img_path"]
    ...
    def __getitem__(self, index):
        ...
        sid = self.ids_list[index % len(self.ids_list)]
        instance = self.train_data[sid]
    ...
    for modality in modalities:
        for resolution in resolutions:
            ...
            res_dict[key] = torch.stack(
                [torch.tensor(example[key]) for example in examples]
            )
...
def instantiate_dataset(
    ...
    train_dataloader = torch.utils.data.DataLoader(
        ...
        collate_fn=lambda examples: collate_fn(
            examples, modalities=modalities, resolutions=resolutios
        ),
...
def validation(
    ...
    for batch in tqdm(val_dataloader, desc="Validation"):
        for modality in args_train.modalities:
            y_org_path = batch["meta_data"][0]["img_path"][
                f"{modality}_{args_train.target_resolution}"
            ]
            ...
            for resolution in args_train.x_valid_resolutions:
                ...
                x_org.append(
                    prep_image.prepare_img(
                        nfc.load_nifti(x_org_path)[0], normalize=True
                    )
                )
            ...
            resolution_one_hot = torch.zeros(
                (x.shape[0], len(args_train.x_valid_resolutions))
            )
            ...
            with torch.no_grad(), torch.amp.autocast("cuda"):
                ...
                for i in range(predictions.shape[0]):
                    ...
                    subject_predictions_2D.append(
                        pred_img[:, :, pred_img.shape[2] // 2]
                    )
            ...
            for res, pred_img in zip(
                args_train.x_valid_resolutions, subject_predictions
            ):
                # compute metrics
            ...
            complete_img.save(
                f"{imgs_2D_output_path}/step_{step}_{modality}_{batch['sid'][0]}.png"
            )
...
def save_model(
    unet, optimizer, lr_scheduler, global_step, out_model_path, ema=None, best=False
...
def train(
    ...
    if args_train.identity_allowed:
        ...
        x_valid_resolutions = [
            res for res in args_train.resolutions if res != args_train.target_resolution
        ]
    args_train.x_valid_resolutions = x_valid_resolutions

    models_dict = instantiate_unconditioned_models(
        device, nb_resolutions=len(x_valid_resolutions)
    )
    ...
    optimizer = torch.optim.Adam(list(unet.parameters()), lr=args_train.lr)
    ...
    # ---- training loop
    progress_bar = tqdm(
        range(0, args_train.max_train_steps), desc="Steps", initial=global_step
    )
    ...
    for epoch in range(first_epoch, max_epochs):
        for batch in train_dataloader:
            ...
            n_batch = len(batch["sid"])

            # create a tensor of index in
            # n_batch = 1

            idx_modalities = torch.randint(
                low=0,
                high=len(args_train.modalities),
                size=(n_batch,),
                generator=gen_modality,
            )
            selected_modalities = [
                args_train.modalities[i] for i in idx_modalities.tolist()
            ]  # this contains n_batch elements

            idx_resolutions = torch.randint(
                low=0,
                high=len(x_valid_resolutions),
                size=(n_batch,),
                generator=gen_resolution,
            )
            selected_resolutions = [
                x_valid_resolutions[i] for i in idx_resolutions.tolist()
            ]  # this contains n_batch elements

            x_keys = [
                f"{mod}_{res}"
                for mod, res in zip(selected_modalities, selected_resolutions)
            ]
            y_keys = [
                f"{mod}_{res}"
                for mod, res in zip(
                    selected_modalities, [args_train.target_resolution] * n_batch
                )
            ]
            ...
            for i, (x_key, y_key) in enumerate(zip(x_keys, y_keys)):
                # print(f"Processing batch {i}, x_key: {x_key}, y_key: {y_key}")
            ...
            with autocast("cuda", enabled=args_train.amp):
                ...
                loss_charbonnier = loss_charbonnier_pt(
                    missing_modality_pred.float(), y.float()
                ...
                loss = (
                    loss_charbonnier / args_train.gradient_accumulation_steps
                )  # Dividir para escalar la pérdida
            ...
            if (
                ...
                if global_step % 10 == 0:
                    ...
                    writer.add_scalar(
                        "Loss_charbonnier/train", loss_charbonnier.item(), global_step
                    )
                ...
                if args_train.initial_val or global_step % args_train.val_interval == 0:
                    ...
                    try:
                        if args_train.use_ema:
                            ema.apply_shadow()
                        validation(
                            unet, autoencoder, global_step, val_dataloader, args_train
                        )
...
args_train = {
    # directories
    "output_path": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test2_direct_dummy_conversion/training/models/to_0.1t/test1_T2FLAIR",
    ...
    "identity_allowed": False,  # if True, the model can learn the identity function, if False, the model has to learn the conversion
    "loss_weights": {
        # "mse": 1.0,
        "charbonnier": 1.0,
        "ssim": 0.5,
