...
def instantiate_unconditioned_models(device, dm_chk_path, dm_seg_channels=3, dm_num_modalitites=3, dm_num_resolutions=3, dm_noise_scheduler_type="rflow", cnet_chk_path=None, cnet_num_resolutions=5, num_inference_steps=30):
...
def validation(
    ...
    tar_resolution_embedding = resolution_encoder_model(tar_resolution_idx-tar_resolution_offset)
    ...
    with torch.no_grad(), torch.amp.autocast("cuda"):
        ...
        latents = latents.squeeze().cpu().numpy()
...
def load_segmentation(segmentation_npy_path, nb_classes=3):
    ...
    return torch.from_numpy(seg_onehot).float().unsqueeze(0)


def get_best_subject_latent_masks(sdf, nb_classes, modalities=["T1W", "T2W", "T2FLAIR"]):
    ...
    for modality in modalities:
        if best_latent_mask[modality] is None:
...
def generate_all_dataset(dataset_df, output_path, models, used_modalities, used_resolutions, nb_classes, tar_resolution_offset=0):
    ...
    for sid in sids:
        ...
        best_latent_mask = get_best_subject_latent_masks(sid_df, nb_classes=nb_classes, modalities=used_modalities)
...
if __name__ == "__main__":
    output_path = "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/data/train_data/generated"
    ...
    generate_all_dataset(dataset_df, 
                         output_path, 
                         models_dict, 
                         used_modalities, 
                         used_resolutions, 
