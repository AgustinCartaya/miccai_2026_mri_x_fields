...
def compute_latent_pred(unet,
    ...
    ############
    # noise_scheduler.set_timesteps(
    #     num_inference_steps=nb_inference_steps,
    #     input_img_size_numel=torch.prod(torch.tensor((48, 64, 48))),
    # )
    # regenerate the image
    with torch.no_grad(), torch.amp.autocast("cuda"):
        # segmentation_embedding = self.models["segmentation_encoder_model"](example["segmentation"].unsqueeze(0).to(device))
        # modality_embedding = modality_encoder_model(example["modality_idx"].to(device))
        # resolution_embedding = resolution_encoder_model(example["resolution_idx"].unsqueeze(0).to(device))
        ...
        # print(f"Latents shape: {latents.shape}, Segmentation shape: {segmentation.shape}, Latent SynthSR shape: {latent_synthsr.shape}")
        # print(f"Modality embedding shape: {modality_embedding.shape}, Resolution embedding shape: {resolution_embedding.shape}")
        for t, next_t in zip(all_timesteps, all_next_timesteps):
            timesteps = torch.full((latents.shape[0],), t, device=device, dtype=torch.long)
            # next_timesteps = torch.full((latents.shape[0],), next_t, device=device, dtype=torch.long)
...
args_train = {
    # directories
    "output_path": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test8_patch_refinement/training/models/segconcatenated_synthsr_mod_res_prior_global_residuals/test1_4steps",
    ...
    # ---- optimizer
    "lr": 2e-5,  # for maisi 1e-3 for maisi 1e-4 # for blsmd 2.5e-5
