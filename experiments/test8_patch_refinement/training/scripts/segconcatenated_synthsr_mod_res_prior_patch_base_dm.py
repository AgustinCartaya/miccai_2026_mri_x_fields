...
def instantiate_unconditioned_models(
    ...
    networks_config = {
        ...
        "residuals_diffusion_unet_def": {
            ...
            "self_attention_levels": [False, False, True],
            "num_self_head_channels": [0, 0, 16],
            "cross_attention_levels": [False, False, False],
            "num_cross_head_channels": [0, 0, 0],
            "use_flash_attention": True,
            "with_conditioning": False,
            "cross_attention_dim": None,
            "transformer_num_layers": 1,  # number of transformer blocks
    ...
    residual_unet = diffusion_model_unet_maisi.DiffusionModelUNetMaisi(
        spatial_dims=args.residuals_diffusion_unet_def.spatial_dims,
        in_channels=args.residuals_diffusion_unet_def.in_channels,
        out_channels=args.residuals_diffusion_unet_def.out_channels,
        num_res_blocks=args.residuals_diffusion_unet_def.num_res_blocks,
        num_channels=args.residuals_diffusion_unet_def.num_channels,
        self_attention_levels=args.residuals_diffusion_unet_def.self_attention_levels,
        cross_attention_levels=args.residuals_diffusion_unet_def.cross_attention_levels,
        num_self_head_channels=args.residuals_diffusion_unet_def.num_self_head_channels,
        num_cross_head_channels=args.residuals_diffusion_unet_def.num_cross_head_channels,
        with_conditioning=args.residuals_diffusion_unet_def.with_conditioning,
        transformer_num_layers=args.residuals_diffusion_unet_def.transformer_num_layers,
        cross_attention_dim=args.residuals_diffusion_unet_def.cross_attention_dim,
        upcast_attention=args.residuals_diffusion_unet_def.upcast_attention,
        use_flash_attention=args.residuals_diffusion_unet_def.use_flash_attention,
        include_top_region_index_input=False,
        include_bottom_region_index_input=False,
        norm_num_groups=8,
    )
    ...
    if noise_scheduler_type == "ddim":
        ...
        dm_noise_scheduler = RFlowScheduler(
            num_train_timesteps=args.noise_scheduler_rf.num_train_timesteps,
            use_discrete_timesteps=args.noise_scheduler_rf.use_discrete_timesteps,
            use_timestep_transform=args.noise_scheduler_rf.use_timestep_transform,
            sample_method=args.noise_scheduler_rf.sample_method,
        ...
        dm_noise_scheduler.set_timesteps(
            num_inference_steps=dm_nb_inference_steps,
            input_img_size_numel=torch.prod(torch.tensor((48, 64, 48))),
        )
    ...
    return {
        ...
        "dm_noise_scheduler": dm_noise_scheduler,
...
class LoadPaths:
    ...
    def get_data(self, split="train"):
        ...
        for i, row in complete_df.iterrows():
            ...
            instance_dict["gen_img_path"] = row["gen_img_path"]
...
class PrepareDataset(Dataset):
    ...
    def __random_crop(self, volume_list):
        _, D, H, W = volume_list[0].shape
    ...
    def random_crop(self, volume, segmentation, gen_img):
        counter = 0
        return_pathch = False
        while True:
            patch_list = self.__random_crop([volume, segmentation, gen_img])
            patch_img, patch_seg, patch_gen_img = patch_list
            ...
            if return_pathch:
                return patch_img, patch_seg, patch_gen_img
    ...
    def __getitem__(self, index):
        ...
        #     example["latent_segmentation"] = self.load_pondered_segmentation(
        #         instance["latent_segmentation_path"],
        #         instance["modality"],
        #         example["subject_id"],
        #     )
        # else:
        #     example["latent_segmentation"] = self.load_segmentation(
        #         instance["latent_segmentation_path"]
        #     )
        ...
        seg_path = instance["segmentation_path"]
        img_gen_path = instance["gen_img_path"]
        ...
        full_gen_img = self.load_image(img_gen_path)

        if self.use_pondered_segmentation:
            full_seg = self.load_pondered_segmentation(
                seg_path,
                instance["modality"],
                example["subject_id"],
            )
        else:
            full_seg = self.load_segmentation(
                instance[seg_path]
            )
        ...
        # SAMPLE PATCH
        # -----------------------
        img_patch, seg_patch, gen_img_patch = self.random_crop(full_img, full_seg, full_gen_img)
        
        example["patch_image"] = img_patch
        example["patch_segmentation"] = seg_patch
        example["patch_gen_image"] = gen_img_patch
...
def collate_fn(examples):
    ...
    resolution_idx = torch.stack([example["resolution_idx"] for example in examples])
    resolution_idx = resolution_idx.to(memory_format=torch.contiguous_format).long()
    res_dict["resolution_idx"] = resolution_idx
    ...
    # latent_segmentation = torch.stack([example["latent_segmentation"] for example in examples])
    # latent_segmentation = latent_segmentation.to(memory_format=torch.contiguous_format).float()
    # res_dict["latent_segmentation"] = latent_segmentation


    patch_image = torch.stack([example["patch_image"] for example in examples])
    patch_image = patch_image.to(memory_format=torch.contiguous_format).float()
    res_dict["patch_image"] = patch_image

    patch_segmentation = torch.stack([example["patch_segmentation"] for example in examples])
    patch_segmentation = patch_segmentation.to(memory_format=torch.contiguous_format).float()
    res_dict["patch_segmentation"] = patch_segmentation

    patch_gen_image = torch.stack([example["patch_gen_image"] for example in examples])
    patch_gen_image = patch_gen_image.to(memory_format=torch.contiguous_format).float()
    res_dict["patch_gen_image"] = patch_gen_image
...
def validation(
    ...
    for batch in val_dataloader:
        ...
        prev_sid = batch["subject_id"][0] if prev_sid is None else prev_sid
        prev_modality = batch["modality"][0] if prev_modality is None else prev_modality
                
        if prev_sid != batch["subject_id"][0] or prev_modality != batch["modality"][0]:
            save_subject_img_list = True
            
        if save_subject_img_list:
            save_2D_images(
                org_img_list + subject_img_list,
                os.path.join(
                    output_path_2D_images,
                    f"imgs2D_step_{step}_{prev_sid}_{prev_modality}.png",
                )
            )
            prev_sid = batch["subject_id"][0]
            prev_modality = batch["modality"][0]
        ...
        for seed in args.val_seeds:
            ...
            latent_residuals = torch.randn(_l_shape, generator=gen_randn).half().to(device)
            gen_randn_prediction = torch.Generator().manual_seed(seed)
            ...
            # latent_pred = batch["latent_pred"].to(device)
            ...
            progress_bar = tqdm(
                ...
                desc=f"Step {step} seed-{seed} {batch['subject_id']} {batch['modality']} {batch['resolution']} {c+1}/{total_images}",
            )
            c += 1
            with torch.no_grad(), torch.amp.autocast("cuda"):

                _lpred_dict = compute_latent_pred(
                    unet=unet,
                    noise_scheduler=dm_noise_scheduler,
                    segmentation=batch["segmentation"].to(device),
                    latent_synthsr=batch["latent_synthsr"].to(device),
                    modality_embedding=modality_embedding,
                    resolution_embedding=resolution_embedding,
                    gen_randn=gen_randn_prediction,
                )

                latent_pred = _lpred_dict["latent_pred"]#.to(device)            

                for t, next_t in progress_bar:

                    model_output = residual_unet(
                        x=torch.cat([latent_residuals, latent_pred], dim=1),
                    ...
                    if not isinstance(noise_scheduler, RFlowScheduler):
                        latent_residuals, _ = noise_scheduler.step(model_output, t, latent_residuals)
                    else:
                        latent_residuals, _ = noise_scheduler.step(model_output, t, latent_residuals, next_t)  # type: ignore

                # free memory for the autoencoder
                del model_output
                torch.cuda.empty_cache()

                latents = latent_pred +latent_residuals
...
def save_model(residual_unet, optimizer, lr_scheduler, global_step, out_model_path, ema=None, best=False):  # MOD: se añade parámetro ema
    # Guardar el modelo
    residual_unet_state_dict = residual_unet.module.state_dict() if torch.distributed.is_initialized() else residual_unet.state_dict()
    checkpoint = {
        "residual_unet_state_dict": residual_unet_state_dict,
    ...
    del checkpoint, residual_unet_state_dict
    torch.cuda.empty_cache()
    gc.collect()


def load_checkpoint(checkpoint_path, residual_unet, device, train_dataloader_len,
    ...
    residual_unet.load_state_dict(checkpoint["residual_unet_state_dict"], strict=False)

    # 3. Move models to GPU
    residual_unet.to(device)
...
def train(
    ...
    gen_randn_prediction = torch.Generator().manual_seed(args_train.seed)
    ...
    dm_noise_scheduler = models_dict["dm_noise_scheduler"]

    residual_unet = models_dict["residual_unet"]
    ...
    for epoch in range(first_epoch, max_epochs):
        for batch in train_dataloader:
            ...
            # segmentation = batch["segmentation"].to(device)
            # latents_synthsr = batch["latent_synthsr"].to(device)
            ...
            with autocast("cuda", enabled=args_train.amp):
                ...
                _lpred_dict = compute_latent_pred(
                    ...
                    resolution_embedding=resolution_embedding,
                    gen_randn=gen_randn_prediction,
                    real_latents=batch["latent"].to(device)
                )

                latent_pred = _lpred_dict["latent_pred"].to(device)
                latents_residuals = _lpred_dict["latent_residuals"].to(device)
            

                # generate noise and timesteps with dedicate generatos and in the cpu for reproducibility
                noise = torch.randn(latents_residuals.shape, device="cpu", generator=gen_noise).to(device)
                if isinstance(noise_scheduler, RFlowScheduler):
                    timesteps = noise_scheduler.sample_timesteps(latents_residuals)
                else:
                    timesteps = torch.randint(0, noise_scheduler.num_train_timesteps, (latents_residuals.shape[0],), device="cpu", generator=gen_t).long().to(device)

                noisy_latent = noise_scheduler.add_noise(original_samples=latents_residuals, noise=noise, timesteps=timesteps)



                model_output = residual_unet(
                    torch.cat([noisy_latent, latent_pred], dim=1),
                    timesteps=timesteps,
            ...
            if (
                ...
                res_norm = latents_residuals.square().mean().sqrt()
                writer.add_scalar("Residual/norm", res_norm.item(), global_step)
                # update writter
                if global_step % 10 == 0:
                ...
                if args_train.initial_val or global_step % args_train.val_interval == 0:
                    ...
                    if args_train.latents_shape is None:
                        args_train.latents_shape = latents_residuals.shape[
                            -4:
...
args_train = {
    # directories
    "output_path": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test8_patch_refinement/training/models/segconcatenated_synthsr_mod_res_prior_global_residuals/test1_4steps_2_coninuation",
    ...
    # "lr_scheduler": {
    #     "name": "WarmupCosineLR",
    #     "warmup_start_factor": 1e-1, # strat is this times the base lr, so 1e-3 means if base lr is 1e-4, qnd this is 1e-2, then the warmup will start at 1e-7 and go to 1e-4 in warmup_steps
    #     "warmup_steps": 100,
    #     "eta_min": 1e-6,
    # },
    ...
    "load_pretrained_model_from": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test8_patch_refinement/training/models/segconcatenated_synthsr_mod_res_prior_global_residuals/test1_4steps_2/check_points/model_1000.pt",
    "resume_from_checkpoint_path_name":None,  
    # "resume_from_checkpoint_path_name":"/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test8_patch_refinement/training/models/segconcatenated_synthsr_mod_res_prior_global_residuals/test1_4steps/check_points/model_10_best.pt",  
