...
__all__ = ["ModelUNetResnetEmbedding"]


class ModelUNetResnetEmbedding(nn.Module):
    """
    ...
    def __init__(
        ...
        if with_conditioning is True and cross_attention_dim is None:
            raise ValueError(
                "ModelUNetResnetEmbedding expects dimension of the cross-attention conditioning (cross_attention_dim) "
                "when using with_conditioning."
            )
        if cross_attention_dim is not None and with_conditioning is False:
            raise ValueError(
                "ModelUNetResnetEmbedding expects with_conditioning=True when specifying the cross_attention_dim."
            )
        ...
        if any((out_channel % norm_num_groups) != 0 for out_channel in num_channels):
            raise ValueError(
                f"ModelUNetResnetEmbedding expects all num_channels being multiple of norm_num_groups, "
        ...
        if len(num_channels) != len(self_attention_levels) or len(num_channels) != len(cross_attention_levels):
            raise ValueError(
                f"ModelUNetResnetEmbedding expects num_channels being same size of attention_levels, "
        ...
        # print(f"dif unet maisi, self.time_embed: {self.time_embed}")
        ...
        #     # print(f"dif unet maisi, self.top_region_index_layer: {self.top_region_index_layer}")
        #     new_time_embed_dim += time_embed_dim
        # if self.include_bottom_region_index_input:
        #     self.bottom_region_index_layer = self._create_embedding_module(4, time_embed_dim)
        #     # print(f"dif unet maisi, self.bottom_region_index_layer: {self.bottom_region_index_layer}")
        #     new_time_embed_dim += time_embed_dim
        # if self.include_spacing_input:
        #     self.spacing_layer = self._create_embedding_module(3, time_embed_dim)
        #     # print(f"dif unet maisi, self.spacing_layer: {self.spacing_layer}")
        #     new_time_embed_dim += time_embed_dim
