...
def compute_latent_pred(unet,
    ...
    with torch.no_grad(), torch.amp.autocast("cuda"):
        ...
        latents_shape = latent_synthsr.shape
        gen_randn = torch.Generator().manual_seed(42)  # fixed seed for reproducibility
        latents = torch.randn(latents_shape, generator=gen_randn).half().to(device)
            
        for t, next_t in zip(all_timesteps, all_next_timesteps):
            timesteps = torch.full((latents.shape[0],), t, device=device, dtype=torch.long)
            model_output = unet(
                x=torch.cat(
                    [latents, segmentation, latent_synthsr],
                    dim=1,
                ),
                timesteps=timesteps,
                modallity_embedding=modality_embedding,
        ...
        return {
            "latent_pred": latents,
            "latent_residuals": (real_latents - latents) if real_latents is not None else None
        }
...
def collate_fn(examples):
    ...
    # latent_pred = torch.stack([example["latent_pred"] for example in examples])
    # latent_pred = latent_pred.to(memory_format=torch.contiguous_format).float()
    # res_dict["latent_pred"] = latent_pred

    # latent_residuals = torch.stack([example["latent_residuals"] for example in examples])
    # latent_residuals = latent_residuals.to(memory_format=torch.contiguous_format).float()
    # res_dict["latent_residuals"] = latent_residuals
