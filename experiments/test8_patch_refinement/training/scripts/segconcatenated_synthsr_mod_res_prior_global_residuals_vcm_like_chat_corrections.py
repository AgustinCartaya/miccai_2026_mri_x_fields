...
class PrepareDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if load_latent_synthsr:
            ...
            example["src_latent_synthsr"] = torch.zeros_like(example["tar_latent"])
...
# # for the VCM
# src_t1w_latent = batch["src_latent_t1w"].to(device)
# src_t2w_latent = batch["src_latent_t2w"].to(device)
# src_flair_latent = batch["src_latent_flair"].to(device)
# tar_condition_resolution_idx = batch["tar_resolution_idx"].to(device)
...
def validation(
    ...
    for batch in val_dataloader:
        ...
        for seed in args.val_seeds:
            ...
            src_resolution_embedding = resolution_encoder_model(src_resolution_idx)
...
def train(
    ...
    train_dataloader = instantiate_dataset(
        ...
        src_resolutions=args_train.used_resolutions,
        tar_resolutions=args_train.used_resolutions,
        gen_modality=gen_modality,
        gen_resolution=gen_resolution,
        ...
        for batch in train_dataloader:
            ...
            if (
                ...
                logs = {"0loss": loss.detach().item(), 
                        "0ln": loss_noise.detach().item(),
                        "0l1": L1_loss.detach().item(),
