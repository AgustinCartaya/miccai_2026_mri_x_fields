...
import networks_declaration.model_unet_resnet_embedding as model_unet_resnet_embedding
...
def instantiate_unconditioned_models(
    ...
    networks_config = {
        "diffusion_unet_def": {
            ...
            "num_self_head_channels": [0,  16],
            "cross_attention_levels": [False, False],
            "num_cross_head_channels": [0, 0],
    ...
    # unet
    unet = model_unet_resnet_embedding.ModelUNetResnetEmbedding(
...
class PrepareDataset(Dataset):
    ...
    def load_segmentation(self, segmentation_npy_path, return_mask=False):
        ...
        if return_mask:
            mask = np.where(segmentation > 0, 1.0, 0.0)
            mask = torch.from_numpy(mask).unsqueeze(0).float()  # (1, D, H, W)
            return torch.from_numpy(seg_onehot).float(), mask
        else:
            return torch.from_numpy(seg_onehot).float()


    def load_pondered_segmentation(
        self, segmentation_npy_path, current_modality, subject_id, return_mask=False
    ):
        seg_list = []
        ...
        if return_mask:
            mask = torch.where(seg_pondered.sum(dim=0, keepdim=True) > 0, 1.0, 0.0)
            return seg_pondered, mask
        return seg_pondered
    ...
    def load_image(self, path):
        ...
        img = util.robust_normalize(img, strictly_positive=True)
        img = torch.from_numpy(img).unsqueeze(0)  # (1,D,H,W)
        return img
    ...
    def __getitem__(self, index):
        ...
        img_gen_path = instance["synthsr_path"]
        ...
        # if self.use_pondered_segmentation:
        #     full_seg, bmask = self.load_pondered_segmentation(
        #         seg_path,
        #         instance["modality"],
        #         example["subject_id"],
        #         return_mask=True
        ...
        #         instance[seg_path]
        #     )
        full_seg = full_img # to delete
        bmask = torch.where(full_img > 0, 1.0, 0.0)
        # skull st
        # ripping the image and the generated image using the mask
        full_gen_img = full_gen_img * bmask
        # -----------------------
        # SAMPLE PATCH
        # -----------------------
        if self.split == "train":
            ...
            img_patch, seg_patch, gen_img_patch = full_img, full_seg, full_gen_img

        example["patch_image"] = img_patch.detach().float()
        example["patch_segmentation"] = seg_patch.detach().float()
        example["patch_gen_image"] = gen_img_patch.detach().float()
...
def validation(
    ...
    for batch in val_dataloader:
        ...
        org_img = batch["patch_image"].squeeze().numpy()
        # org_seg = batch["patch_segmentation"]
        gen_img = batch["patch_gen_image"].to(device)
        ...
        for seed in args.val_seeds:
            ...
            with torch.amp.autocast("cuda"):
                ...
                model_pred = sliding_window_inference(
                    ...
                    sw_device=gen_img.device,
                    device=gen_img.device,
                    progress=True,
                )
                
                # synthetic_images = model_pred + gen_img
                synthetic_images = model_pred
                synthetic_images = synthetic_images.squeeze().cpu().numpy()  # (B, 1, D, H, W) -> (B, D, H, W)
                del model_pred, gen_img, modality_embedding, resolution_embedding
                # torch.cuda.empty_cache()
                # model_output = unet(
                #     x=torch.cat([org_gen_img], dim=1),
                #     modallity_embedding=modality_embedding,
                #     resolution_embedding=resolution_embedding,
                # )
...
def train(
    ...
    train_dataloader = instantiate_dataset(
        ...
        patch_size=args_train.patch_size,
        ensure_info_percent=args_train.ensure_info_percent
    )
    ...
    for epoch in range(first_epoch, max_epochs):
        for batch in train_dataloader:
            ...
            # prepare inputs
            org_patch = batch["patch_image"].to(device)
            gen_patch = batch["patch_gen_image"].to(device)
            ...
            with autocast("cuda", enabled=args_train.amp):
                ...
                # model_gt = org_patch - gen_patch  # target is the residual between the original and generated patch
                model_gt = org_patch  # target is the residual between the original and generated patch

            model_output = model_output.float()
            model_gt = model_gt.float()
            # reconstructed_patch = model_output + gen_patch
            # reconstructed_patch = model_output + gen_patch

            loss_mse = F.l1_loss(model_output, org_patch)  # Dividir para escalar la pérdida
            # loss_perceptual = loss_perceptual_pt(reconstructed_patch, org_patch)
            loss_ssim = loss_ssim_pt(model_output, org_patch)
            # loss_lap = laplacian_loss(reconstructed_patch, org_patch)
            
            loss = (
                loss_mse + \
                # args_train.loss_weights.perceptual * loss_perceptual + \
                args_train.loss_weights.ssim * loss_ssim 
                # args_train.loss_weights.lap * loss_lap
            ...
            if (
                ...
                if global_step % 10 == 0:
                    ...
                    res_norm = model_output.square().mean().sqrt()
                    magnitude = (model_output / model_gt).square().mean().sqrt()
                    writer.add_scalar("Residual/norm", res_norm.item(), global_step)
                    writer.add_scalar("Magnitude/delta_norm", magnitude.item(), global_step)
                ...
                if args_train.initial_val or global_step % args_train.val_interval == 0: #or (global_step < 1000 and global_step % 25 == 0):
                    unet.eval()
...
args_train = {
    # directories
    "output_path": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test8_patch_refinement/training/models/segconcatenated_synthsr_mod_res_prior_patch_base/test1_from_synthsr_no_residuals",
    ...
    "val_dataset_filters": {
        ...
        # "modality": ["T1W"],  # Example filter for specific modalities
        # "resolution": [7],  # Example filter for specific resolutions
    },
    ...
    "patch_size": [64, 64, 64],
    "ensure_info_percent": 0.15,  # minimum percentage of non-zero vox
