...
import networks_declaration.vcm_diffusion_model_unet_maisi_mask_seg as vcm_diffusion_model_unet_maisi
...
def instantiate_unconditioned_models(
    ...
    networks_config = {
        ...
        "vcm_unet_def": {
            ...
            "spatial_dims": 3,
            # "in_channels": 4 + 4 + 4*3,
            "in_channels": 4 + 4 + 4 + nb_seg_classes,
            "out_channels": 4,
            "num_res_blocks": 2,
            "num_channels": [
                    64,
            ...
            "num_self_head_channels": [0, 0, 256],
    ...
    vcm_unet = vcm_diffusion_model_unet_maisi.VCMDiffusionModelUNetMaisi(
        spatial_dims=args.vcm_unet_def.spatial_dims,
        in_channels=args.vcm_unet_def.in_channels,
        out_channels=args.vcm_unet_def.out_channels,
        num_res_blocks=args.vcm_unet_def.num_res_blocks,
        num_channels=args.vcm_unet_def.num_channels,
        self_attention_levels=args.vcm_unet_def.self_attention_levels,
        cross_attention_levels=args.vcm_unet_def.cross_attention_levels,
        num_self_head_channels=args.vcm_unet_def.num_self_head_channels,
        num_cross_head_channels=args.vcm_unet_def.num_cross_head_channels,
        with_conditioning=args.vcm_unet_def.with_conditioning,
        transformer_num_layers=args.vcm_unet_def.transformer_num_layers,
        cross_attention_dim=args.vcm_unet_def.cross_attention_dim,
        upcast_attention=args.vcm_unet_def.upcast_attention,
        use_flash_attention=args.vcm_unet_def.use_flash_attention,
        include_top_region_index_input=False,
        include_bottom_region_index_input=False,
        norm_num_groups=8,
    )
...
# tar_condition_resolution_idx = batch["tar_resolution_idx"].to(device)
...
def validation(
    ...
    total_images = len(args.val_seeds) * len(val_dataloader) * 2
    c = 0
    val_results = []
    subject_img_list = []
    org_img_list = []
    ...
    for batch in val_dataloader:
        ...
        for seed in args.val_seeds:
            ...
            for use_synthsr in [True]:
                ...
                modality_embedding = modality_encoder_model(
                    batch["modality_idx"].to(device)
                )
                resolution_embedding = resolution_encoder_model(
                    batch["resolution_idx"].to(device)
                )
                ...
                latents_synthsr = batch["latent_synthsr"]
                if use_synthsr:
                    latents_synthsr = latents_synthsr.to(device)
                else:
                ...
                with torch.no_grad(), torch.amp.autocast("cuda"):
                    ...
                    for t, next_t in progress_bar:
                        ...
                        model_output = unet(
                            # x=latents,
                            x=torch.cat([latents, segmentation, latents_synthsr], dim=1),
                            timesteps=torch.Tensor((t,)).to(device),
                        ...
                        torch.cat([latents, model_output, segmentation, latents_synthsr], dim=1),
                        ...
                        # gamma_t = torch.nn.Sigmoid()(gamma_t)
                        gamma_t = 0.1 * torch.tanh(gamma_t)
                        model_output = model_output * (1 + gamma_t) + beta_t
...
def save_model(vcm_model, optimizer, lr_scheduler, global_step, out_model_path, ema=None, best=False):  # MOD: se añade parámetro ema
    # Guardar el modelo
    vcm_model_state_dict = vcm_model.module.state_dict() if torch.distributed.is_initialized() else vcm_model.state_dict()
    checkpoint = {
        "vcm_model_state_dict": vcm_model_state_dict,
    ...
    del checkpoint, vcm_model_state_dict
    torch.cuda.empty_cache()
    gc.collect()


def load_checkpoint(checkpoint_path, vcm_model, device, train_dataloader_len,
    ...
    vcm_model.load_state_dict(checkpoint["vcm_model_state_dict"], strict=False)
...
def train(
    ...
    vcm_model = models_dict["vcm_model"]
    ...
    for epoch in range(first_epoch, max_epochs):
        for batch in train_dataloader:
            ...
            # prepare inputs
            tar_latents = batch["latent"].to(device)
            condition_modality_idx = batch["modality_idx"].to(device)
            tar_condition_resolution_idx = batch["resolution_idx"].to(device)


            src_segmentation = batch["segmentation"].to(device)
            src_latents_synthsr = batch["latent_synthsr"].to(device)
            ...
            with autocast("cuda", enabled=args_train.amp):
                ...
                tar_resolution_embedding = resolution_encoder_model(tar_condition_resolution_idx)
                # src_resolution_embedding = resolution_encoder_model(src_condition_resolution_idx)
                ...
                gamma_t, beta_t = vcm_model(
                    torch.cat([noisy_latent, model_output, src_segmentation, src_latents_synthsr], dim=1),
                    ...
                    modallity_embedding=modality_embedding,
                    resolution_embedding=tar_resolution_embedding,
                    # src_resolution_embedding=src_resolution_embedding,
                )
                # gamma_t = torch.nn.Sigmoid()(gamma_t)
                gamma_t = 0.1 * torch.tanh(gamma_t)


                epsilon_p_t = model_output * (1 + gamma_t) + beta_t
            ...
            loss_noise = loss_pt(epsilon_p_t.float(), model_gt.float())  # Dividir para escalar la pérdida
            lambda_x = 1

            L1_loss = torch.norm(gamma_t.float(), p=1) + torch.norm(beta_t.float(), p=1)
            BZ, c, h, w, d = noisy_latent.size()
            lambda_1 = 1 / float((BZ * c * h * w * d))  # Normalizar la pérdida L1 por el tamaño del batch y las dimensiones de la imagen

            loss = lambda_x * loss_noise + lambda_1 * L1_loss
            ...
            if (
                ...
                res_norm = tar_latents.square().mean().sqrt()
                ...
                if global_step % 10 == 0:
                    ...
                    magnitude_gamma = gamma_t.abs().mean()
                    magnitude_beta = beta_t.abs().mean()
                    magnitude_delta = (epsilon_p_t - model_output).abs().mean()

                    writer.add_scalar("Magnitude/gamma", magnitude_gamma.item(), global_step)
                    writer.add_scalar("Magnitude/beta", magnitude_beta.item(), global_step)
                    writer.add_scalar("Magnitude/delta", magnitude_delta.item(), global_step)
                    writer.add_scalar("Magnitude/delta_norm", (magnitude_delta.item()/model_output.abs().mean())*100, global_step)
                ...
                if args_train.initial_val or global_step % args_train.val_interval == 0:
                    ...
                    if args_train.latents_shape is None:
                        args_train.latents_shape = tar_latents.shape[
                            -4:
...
args_train = {
    # directories
    "output_path": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test8_patch_refinement/training/models/segconcatenated_synthsr_mod_res_prior_vcm_no_paired_data/test2",
    ...
    "lr_scheduler": {
        "name": "WarmupCosineLR",
        "warmup_start_factor": 1e-2, # strat is this times the base lr, so 1e-3 means if base lr is 1e-4, qnd this is 1e-2, then the warmup will start at 1e-7 and go to 1e-4 in warmup_steps
        "warmup_steps": 250,
        "eta_min": 1e-5,
    },
