...
# from tests.D3.maisi.understanding_training.conditioned_aaco5590_zscore.networks_declaration.diffusion_model_unet import (
#     get_down_block,
#     get_mid_block,
...
class SimpleConditionEmbedding(nn.Module):
    # this embedding is used to concatenate with the time embedding
    def __init__(self, num_conditions=3, embed_dim=512):
        super().__init__()
...
class DiffusionModelUNetMaisi(nn.Module):
    ...
    def __init__(
        ...
        self_attention_levels: Sequence[bool] = (False, False, True, True),
        cross_attention_levels: Sequence[bool] = (False, False, True, True),
        norm_num_groups: int = 32,
        norm_eps: float = 1e-6,
        resblock_updown: bool = False,
        # num_head_channels: int | Sequence[int] = 8,
        num_self_head_channels: int | Sequence[int] = 8,
        num_cross_head_channels: int | Sequence[int] = 8,
        ...
        num_src_resolutions: int = 1,
        num_tar_resolutions: int = 1,
    ) -> None:
        ...
        if len(num_channels) != len(self_attention_levels) or len(num_channels) != len(cross_attention_levels):
            raise ValueError(
                f"DiffusionModelUNetMaisi expects num_channels being same size of attention_levels, "
                f"but get num_channels: {len(num_channels)} and self_attention_levels: {len(self_attention_levels)} and cross_attention_levels: {len(cross_attention_levels)}"
            )

        if isinstance(num_self_head_channels, int):
            num_self_head_channels = ensure_tuple_rep(num_self_head_channels, len(self_attention_levels))

        if isinstance(num_cross_head_channels, int):
            num_cross_head_channels = ensure_tuple_rep(num_cross_head_channels, len(cross_attention_levels))

        if len(num_self_head_channels) != len(self_attention_levels) or len(num_cross_head_channels) != len(cross_attention_levels):
            raise ValueError(
        ...
        self.self_attention_levels = self_attention_levels
        self.cross_attention_levels = cross_attention_levels
        self.num_self_head_channels = num_self_head_channels
        self.num_cross_head_channels = num_cross_head_channels
        ...
        # self.src_resolution_embedding = SimpleConditionEmbedding(num_conditions=num_src_resolutions, embed_dim=time_embed_dim)
        # self.tar_resolution_embedding = SimpleConditionEmbedding(num_conditions=num_tar_resolutions, embed_dim=time_embed_dim)
        # self.modality_embedding = SimpleConditionEmbedding(num_conditions=num_modalities, embed_dim=time_embed_dim)
        # self.availables_modality_embedding = self._create_embedding_module(num_modalities, time_embed_dim)
        ...
        self.src_resolution_embedding = SimpleConditionEmbedding(4, time_embed_dim)
        self.tar_resolution_embedding = SimpleConditionEmbedding(4, time_embed_dim)
        self.modality_embedding = SimpleConditionEmbedding(3, time_embed_dim)
        self.availables_modality_embedding = self._create_embedding_module(3, time_embed_dim)

        new_time_embed_dim = time_embed_dim * 5
        ...
        for i in range(len(num_channels)):
            ...
            down_block = get_down_block(
                ...
                with_self_attn=(self_attention_levels[i] and not with_conditioning),
                with_cross_attn=(cross_attention_levels[i] and with_conditioning),
                num_self_head_channels=num_self_head_channels[i],
                num_cross_head_channels=num_cross_head_channels[i],
        ...
        self.middle_block = get_mid_block(
            ...
            norm_num_groups=norm_num_groups,
            norm_eps=norm_eps,
            with_conditioning=(cross_attention_levels[-1] and with_conditioning),
            num_self_head_channels=num_self_head_channels[-1],
            num_cross_head_channels=num_cross_head_channels[-1],
            transformer_num_layers=transformer_num_layers,
            cross_attention_dim=cross_attention_dim,
            upcast_attention=upcast_attention,
            include_fc=include_fc,
        ...
        reversed_self_attention_levels = list(reversed(self_attention_levels))
        reversed_cross_attention_levels = list(reversed(cross_attention_levels))
        reversed_num_self_head_channels = list(reversed(num_self_head_channels))
        reversed_num_cross_head_channels = list(reversed(num_cross_head_channels))
        ...
        for i in range(len(reversed_block_out_channels)):
            ...
            up_block = get_up_block(
                ...
                resblock_updown=resblock_updown,
                with_self_attn=(reversed_self_attention_levels[i] and not with_conditioning),
                with_cross_attn=(reversed_cross_attention_levels[i] and with_conditioning),
                num_self_head_channels=reversed_num_self_head_channels[i],
                num_cross_head_channels=reversed_num_cross_head_channels[i],
                transformer_num_layers=transformer_num_layers,
                cross_attention_dim=cross_attention_dim,
                upcast_attention=upcast_attention,
                include_fc=include_fc,
    ...
    def _get_time_and_class_embedding(self, x, timesteps, class_labels):
        t_emb = get_timestep_embedding(timesteps, self.block_out_channels[0]) # sinusoidal embedding
    ...
    def _get_input_embeddings(self, emb, modallity_idx, src_resolution_idx, tar_resolution_idx, availables_modalities_mask):
        emb_modality = self.modality_embedding(modallity_idx)
        emb_src_resolution = self.src_resolution_embedding(src_resolution_idx)
        emb_tar_resolution = self.tar_resolution_embedding(tar_resolution_idx)
        emb_availables_modalities = self.availables_modality_embedding(availables_modalities_mask)

        emb = torch.cat((emb, emb_modality, emb_src_resolution, emb_tar_resolution, emb_availables_modalities), dim=1)
        return emb
    ...
    def forward(
        ...
        modallity_idx: torch.Tensor | None = None,
        src_resolution_idx: torch.Tensor | None = None,
        tar_resolution_idx: torch.Tensor | None = None,
        availables_modalities_mask: torch.Tensor | None = None,
    ) -> torch.Tensor:
        """
        ...
        emb = self._get_input_embeddings(emb, modallity_idx, src_resolution_idx, tar_resolution_idx, availables_modalities_mask)

        # if availables_modalities_mask is not None:
        #     availables_modality_embedding = self.availables_modality_embedding(availables_modalities_mask)
        #     emb += availables_modality_embedding
        # if modallity_idx is not None:
        #     modallity_embedding = self.modality_embedding(modallity_idx)
        #     emb += modallity_embedding
        # if src_resolution_idx is not None:
        #     src_resolution_embedding = self.src_resolution_embedding(src_resolution_idx)
        #     emb += src_resolution_embedding
        # if tar_resolution_idx is not None:
        #     tar_resolution_embedding = self.tar_resolution_embedding(tar_resolution_idx)
        #     emb += tar_resolution_embedding
