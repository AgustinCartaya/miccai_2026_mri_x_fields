...
def validation(
    ...
    org_images = [None] * len(val_dataloader)
    # metrics = {"ssim": [], "rmse": []}
    val_results = []
    for seed_idx, seed in enumerate(args.val_seeds):
        ...
        for subject_idx, batch in enumerate(val_dataloader):
            ...
            # load org image for comparison
            if org_images[subject_idx] is None:
                org_img = nfc.load_nifti(batch["org_img_path"][0])[0]
                org_img = prep_image.prepare_img(org_img)
                org_images[subject_idx] = org_img
            
            seed_img_list.append(synthetic_images)

            # compute metrics
            metrics_dict = compute_image_metrics(synthetic_images, org_images[subject_idx])
            ...
            # seed_img_list.append(org_images[subject_idx])  # add the original image for comparison
    ...
    imgs_list = org_images + imgs_list
...
def train(
    ...
    train_dataloader = instantiate_dataset(
        ...
        used_modalities=args_train.finetune_modalities,
        used_resolutions=args_train.finetune_resolutions,
...
args_train = {
    # directories
    "output_path": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test5_segmentation_prior/training/models/all_357t/segconcatenated/finetuning/test1_2_T1W_7T",
    ...
    "val_seeds": [0, 2,10,42,12357],  # [0,12357], # seeds for the noise generation during validation
