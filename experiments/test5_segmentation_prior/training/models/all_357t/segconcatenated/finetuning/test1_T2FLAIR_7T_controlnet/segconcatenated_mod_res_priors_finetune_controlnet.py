...
def instantiate_unconditioned_models(
    ...
    networks_config =  {
        ...
        "controlnet_def": {
            ...
            "num_class_embeds": nb_resolutions  # this is the number of modalities that we have as conditions
        },
        ...
        for subject_id in self.sids:
            ...
            for modality in self.used_modalities:
                ...
                for resolution in self.used_resolutions:
                    ...
                    _instance["resolution_idx"] = row["resolution_idx"]
...
class PrepareDataset(Dataset):
    def __init__(
        ...
        self.used_modalities = used_modalities
        self.used_resolutions = used_resolutions
        self.finetune_modalities = finetune_modalities
        self.finetune_resolutions = finetune_resolutions
        ...
        self.resolution_index_mapping = data_loader.resolution_index_mapping
    ...
    def __getitem__(self, index):
        ...
        if self.split == "train":
            rand_modality_i = torch.randint(len(self.finetune_modalities), (1,), generator=self.gen_modality).item()
            rand_modality = self.finetune_modalities[rand_modality_i]
            modality_idx = self.modality_index_mapping[rand_modality]
            ...
            tar_rand_resolution_i = torch.randint(len(self.finetune_resolutions),(1,),generator=self.gen_resolution).item()
            rand_tar_resolution = self.finetune_resolutions[tar_rand_resolution_i]
            tar_resolution_idx = self.resolution_index_mapping[rand_tar_resolution]
        elif self.split == "val":
            # for validation we will use the val_combinations to ensure that we cover all combinations of modalities and resolutions
            rand_modality, rand_src_resolution, rand_tar_resolution = self.val_combinations[index % len(self.val_combinations)]
            modality_idx = self.modality_index_mapping[rand_modality]
            src_resolution_idx = self.resolution_index_mapping[rand_src_resolution]
            tar_resolution_idx = self.resolution_index_mapping[rand_tar_resolution]

            # # update the counter
            # self.val_combinations_counter += 1
            # if self.val_combinations_counter >= len(self.val_combinations):
            #     self.val_combinations_counter = 0
...
def instantiate_dataset(dataset_path_name, used_modalities, used_resolutions, finetune_modalities, finetune_resolutions,
...
args_train = {
    # directories 
    "output_path": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test5_segmentation_prior/training/models/all_357t/segconcatenated/finetuning/test1_T2FLAIR_7T_controlnet",
    ...
    # ---- Pretrained UNET -----
    "pretrained_models_path": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test5_segmentation_prior/training/models/all_357t/segconcatenated/finetuning/test1_T2FLAIR_7T/check_points/model_6000_best.pt",
    ...
    "val_seeds": [0,42],#[0,12357], # seeds for the noise generation during validation
    ...
    "finetune_modalities": ["T2FLAIR"],  # "T1W", "T2W", "T2FLAIR"
    "finetune_resolutions": [7], 
