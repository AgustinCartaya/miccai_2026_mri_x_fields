...
def instantiate_unconditioned_models(device, noise_scheduler_type="rflow", nb_seg_classes=3, nb_resolutions=5, nb_modalities=3):
...
class LoadPaths:
    ...
    def get_data(self, split="train"):
        ...
        for i, row in complete_df.iterrows():
            ...
            instance_dict["segmentation_npy_path"] = row[f"latent_seg_supersynth_merged_{self.nb_seg_classes}_path"]
...
# def save_model(unet, modality_encoder_model, resolution_encoder_model, optimizer, lr_scheduler, global_step, out_model_path, ema=None, best=False):  # MOD: se añade parámetro ema
def save_model(unet, modality_encoder_model, resolution_encoder_model, discriminator_model, optimizer, optimizer_discriminator, lr_scheduler, lr_scheduler_discriminator, global_step, out_model_path, ema=None, best=False):  # MOD: se añade parámetro ema
    # Guardar el modelo
...
# def load_checkpoint(checkpoint_path, unet, modality_encoder_model, resolution_encoder_model, device, train_dataloader_len,
#                     gradient_accumulation_steps, batch_size, optimizer=None, lr_scheduler=None, ema=None):

def load_checkpoint(checkpoint_path, unet, modality_encoder_model, resolution_encoder_model, discriminator_model, device, train_dataloader_len,
                    gradient_accumulation_steps, batch_size, optimizer=None, optimizer_discriminator=None, lr_scheduler=None, lr_scheduler_discriminator=None, ema=None):
...
def train(
    ...
    for epoch in range(first_epoch, max_epochs):
        for batch in train_dataloader:
            ...
            _, pred_latent = noise_scheduler.step(
                model_output,
                timesteps.float().view(-1, 1, 1, 1, 1),
                noisy_latent,
            )

            # ============================================================
            # ADVERSARIAL TIMESTEP FILTER
            # ============================================================
            ...
            if use_adv:
                ...
                with autocast("cuda", enabled=args_train.amp):
                    ...
                    real_latent = latents[adv_mask].detach()
                    fake_latent = pred_latent[adv_mask].detach()

                    pred_real = discriminator_model(real_latent)[-1]
                    pred_fake = discriminator_model(fake_latent)[-1]

                    loss_real = torch.relu(1.0 - pred_real.float()).mean()
                    loss_fake = torch.relu(1.0 + pred_fake.float()).mean()
            ...
            if use_adv:
                ...
                with autocast("cuda", enabled=args_train.amp):
                    ...
                    pred_fake_g = discriminator_model(pred_latent[adv_mask])[-1]

                    loss_adv = (-pred_fake_g.float().mean())

            loss_generator = (loss_noise + args_train.loss_weights.adv * loss_adv)
            ...
            if (gradient_accumulation_count % args_train.gradient_accumulation_steps == 0):
                ...
                if global_step % 10 == 0:
                    ...
                    writer.add_scalar("Loss_D/train",loss_discriminator.item(),global_step,)
                ...
                if args_train.initial_val or global_step % args_train.val_interval == 0 or (global_step < 1000 and global_step % 100 == 0):
                    unet.eval()
...
args_train = {
    # directories 
    "output_path": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test5_segmentation_prior/training/models/all_357t/segconcatenated/test4_merged8_4res_adversarial_t1w", # model output path, where the checkpoints, logs and validation images will be saved
    ...
    "val_dataset_filters": {
        'sid': ['S0006', 'S0007']  # Example filter for a specific subject ID
        # 'iid': [
        #     "SP_T1W_0.1T_0006", "SP_T1W_0.1T_0006", "SP_T1W_7T_0006", 
        #     "SP_T1W_3T_0006", "SP_T1W_5T_0006", "SP_T1W_7T_0006", 
        #     "SP_T1W_3T_0006", "SP_T1W_5T_0006", "SP_T1W_7T_0006", 
    ...
    "nb_seg_classes": 8, # number of segmentation classes including the background, used for the one hot encoding of the segmentation maps
}
