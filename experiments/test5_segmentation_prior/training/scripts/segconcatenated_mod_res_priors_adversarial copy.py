    ...
    for epoch in range(first_epoch, max_epochs):
        for batch in train_dataloader:
            ...
            with autocast("cuda", enabled=args_train.amp):
                pred_real = discriminator_model(real_latent)[-1].view(-1)
            
            loss_real = criterion(pred_real.float(), label)
            loss_real.backward()
            ...
            with autocast("cuda", enabled=args_train.amp):
                ...
                model_output = unet(
                                torch.cat([noisy_latent, batch["segmentation"].to(device)], dim=1), 
                                    timesteps=timesteps,
                ...
                _, fake_latent = noise_scheduler.step(model_output, timesteps.float().view(-1, 1, 1, 1, 1), noisy_latent)
                pred_fake = discriminator_model(fake_latent.detach())[-1].view(-1)

            label.fill_(fake_label)
            loss_fake = criterion(pred_fake.float(), label)
            loss_fake.backward()
            ...
            pred_fake_g = discriminator_model(fake_latent)[-1].view(-1)
            loss_adv = criterion(pred_fake_g.float(), label)
...
args_train = {
    # directories 
    "output_path": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test5_segmentation_prior/training/models/all_357t/segconcatenated/test3_merged8_4res",
