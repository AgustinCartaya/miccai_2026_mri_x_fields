...
# from tests.D3.maisi.understanding_training.conditioned_aaco5590_zscore.networks_declaration.selfattention import SABlock
# from tests.D3.maisi.understanding_training.conditioned_aaco5590_zscore.networks_declaration.crossattention import CrossAttentionBlock
# from tests.D3.maisi.understanding_training.conditioned_aaco5590_zscore.networks_declaration.spatialattention import SpatialAttentionBlock
...
class DiffusionUNetTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        num_self_head_channels: int,
        num_cross_head_channels: int,
        dropout: float = 0.0,
        ...
        super().__init__()
        if num_self_attention_heads > 0 and num_self_head_channels > 0:
            self.attn1 = SABlock(
                # hidden_size=num_attention_heads * num_head_channels,
                hidden_size=num_self_attention_heads * num_self_head_channels,
                hidden_input_size=num_channels,
                num_heads=num_self_attention_heads,
                dim_head=num_self_head_channels,
        ...
        if num_cross_attention_heads > 0 and num_cross_head_channels > 0:
            self.attn2 = CrossAttentionBlock(
                # hidden_size=num_attention_heads * num_head_channels,
                hidden_size=num_cross_attention_heads * num_cross_head_channels,
                num_heads=num_cross_attention_heads,
                hidden_input_size=num_channels,
                context_input_size=cross_attention_dim,
                dim_head=num_cross_head_channels,
                dropout_rate=dropout,
                attention_dtype=torch.float if upcast_attention else None,
                use_flash_attention=use_flash_attention,
            )
        ...
        # print("nn.LayerNorm(num_channels)", num_channels)
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor | None = None) -> torch.Tensor:
        ...
        # THE PROBLEM IS HERE THE CODE WORKS THAKS TO THE ADITION OF X
        # 2. Cross-Attention
...
class SpatialTransformer(nn.Module):
    ...
    def __init__(
        ...
        if num_cross_attention_heads > 0 and num_cross_head_channels > 0:
            inner_dim = num_cross_attention_heads * num_cross_head_channels
        elif num_self_attention_heads > 0 and num_self_head_channels > 0:
            inner_dim = num_self_attention_heads * num_self_head_channels
        else:
            raise ValueError("Either num_cross_attention_heads and num_cross_head_channels or num_self_attention_heads and num_self_head_channels must be > 0")
        ...
        self.transformer_blocks = nn.ModuleList(
            [
                DiffusionUNetTransformerBlock(
                    ...
                    num_self_head_channels=num_self_head_channels,
                    num_cross_head_channels=num_cross_head_channels,
                    dropout=dropout,
...
class AttnDownBlock(nn.Module):
    ...
    def __init__(
        ...
        # print("instantiating AttnDownBlock")
        super().__init__()
        self.resblock_updown = resblock_updown
...
class CrossAttnDownBlock(nn.Module):
    ...
    def __init__(
        ...
        downsample_padding: int = 1,
        # num_head_channels: int = 1,
        num_self_head_channels: int = 1,
        num_cross_head_channels: int = 1,
        ...
        # print("instantiating CrossAttnDownBlock")
        ...
        for i in range(num_res_blocks):
            ...
            attentions.append(
                SpatialTransformer(
                    ...
                    # num_head_channels=num_head_channels,
                    num_self_attention_heads=out_channels //num_self_head_channels if num_self_head_channels > 0 else 0,
                    num_cross_attention_heads=out_channels // num_cross_head_channels if num_cross_head_channels > 0 else 0,
                    num_self_head_channels=num_self_head_channels,
                    num_cross_head_channels=num_cross_head_channels,
...
class CrossAttnMidBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attention = SpatialTransformer(
            ...
            # num_head_channels=num_head_channels,
            num_self_attention_heads=in_channels //num_self_head_channels if num_self_head_channels > 0 else 0,
            num_cross_attention_heads=in_channels // num_cross_head_channels if num_cross_head_channels > 0 else 0,
            num_self_head_channels=num_self_head_channels,
            num_cross_head_channels=num_cross_head_channels,
            num_layers=transformer_num_layers,
            norm_num_groups=norm_num_groups,
...
class AttnUpBlock(nn.Module):
    ...
    def __init__(
        ...
        # print("instantiating AttnUpBlock")
        super().__init__()
        self.resblock_updown = resblock_updown
...
class CrossAttnUpBlock(nn.Module):
    ...
    def __init__(
        ...
        # print("instantiating CrossAttnUpBlock")
...
def get_down_block(
    ...
    # print("get_down_block, with_attn:", with_attn, "with_cross_attn:", with_cross_attn)
    if with_self_attn:
        # just self attention
        return AttnDownBlock(
            ...
            # num_head_channels=num_head_channels,
            num_head_channels=num_self_head_channels,
            include_fc=include_fc,
            use_combined_linear=use_combined_linear,
            use_flash_attention=use_flash_attention,
        )
    elif with_cross_attn:
        # self and then cross attention
...
def get_mid_block(
    ...
    # print("get_mid_block, with_conditioning:", with_conditioning)
...
def get_up_block(
    ...
    # print("get_up_block, with_attn:", with_attn, "with_cross_attn:", with_cross_attn)
...
class DiffusionModelUNet(nn.Module):
    ...
    def forward(
        ...
        # print(f"dmu: x.shape {x.shape}")
        h = self.conv_in(x)
        # print(f"dmu: h.shape {h.shape}")
        ...
        for downsample_block in self.down_blocks:
            ...
            # print(f"dmu downsample_block: len(h) {len(h)}")
            # print(f"dmu downsample_block: h[0].shape {h[0].shape}")
            # print(f"dmu downsample_block: res_samples.shape {res_samples.shape}")
