...
class CrossAttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        # print(f"in crossattention.py, self.context_input_size: {self.context_input_size}, inner_size: {inner_size}")
    ...
    def forward(self, x: torch.Tensor, context: Optional[torch.Tensor] = None):
        ...
        _, kv_t, _ = kv.size()
        # print(f"in crossattention.py, kv.shape: {kv.shape}")
        k = self.input_rearrange(self.to_k(kv))
        v = self.input_rearrange(self.to_v(kv))

        # print(f"in crossattention.py, q.shape: {q.shape}, k.shape: {k.shape}, v.shape: {v.shape}")
