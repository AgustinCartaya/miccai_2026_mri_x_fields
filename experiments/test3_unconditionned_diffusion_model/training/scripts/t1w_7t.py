...
class LoadPaths:
    def __init__(self, 
        ...
        self.df = self.df[( self.df["modality"] == target_modality) & (self.df["resolution"] == target_resolution) ]
...
def instantiate_dataset(dataset_path_name, target_modality, target_resolution,
                        batch_size, 
                        dataset_filters=None,
                        split="train",
                        ):
...
def validation(
    ...
    imgs_list = []
    total_images = len(args.val_seeds) 
    c = 0
    for seed in args.val_seeds:
        ...
        imgs_list.append(synthetic_images)
...
def train(
    ...
    train_dataloader = instantiate_dataset(
        ...
        target_modality=args_train.target_modality,
        ...
        for batch in train_dataloader:
            ...
            if gradient_accumulation_count % args_train.gradient_accumulation_steps == 0:
                ...
                logs = {"loss": loss.detach().item(), 
                        "id": [f'{__id[:4]}' for __id in batch["subject_id"]], 
                        # **logs_conditions,
                        }
                ...
                if args_train.initial_val or global_step % args_train.val_interval == 0:
                    ...
                    try:
                        if args_train.use_ema:
                            ema.apply_shadow()
                        validation(unet, noise_scheduler, autoencoder, global_step, args_train)
...
args_train = {
    # directories 
    "output_path": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test3_unconditionned_diffusion_model/training/models/t1w_7t/test2",
    ...
    # ---- pretrained_model
    "load_pretrained_model_from": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test3_unconditionned_diffusion_model/training/models/t1w_7t/test1/check_points/model_10000.pt",
    ...
    "val_interval": 1000,
    "initial_val": True,
    "val_seeds": [0,3,7,9,11,15,100], # seeds for the noise generation during validation
