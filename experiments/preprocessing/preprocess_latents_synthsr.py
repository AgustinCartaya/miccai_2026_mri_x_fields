...
def preprocess_latents(split="train", decode_first=False):
    ...
    base_output_path = f"/home/agustin/phd/miccai/miccai_2026/mri_x_fields/data/{split}_data/preprocessed/maisi_latents_synthsr"
    ...
    for index, row in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        # split = row['split']
        img_path = row['synthsr_path']
        seg_path = row['seg_supersynth_path']
        ...
        img[seg == 0] = 0
        img, _, new_aff = fc.resize_center_crop_pad(img, (384, 448, 384), aff)
        ...
        if decode_first:
            ...
            decoded_path_name = output_latent_path_name.replace("_latent.npy", "_decoded.nii.gz")
            nfc.save_nifti(decoded, new_aff, decoded_path_name)
            decode_first = False
            print(f"Decoded first latent for {subject_id} {resolution}T {modality}, saved to {decoded_path_name}")
    ...
    df["latent_synthsr_path"] = latent_paths
    output_csv_path = f"/home/agustin/phd/miccai/miccai_2026/mri_x_fields/data/csv/{split}_data_with_latents_synthsr.csv"
