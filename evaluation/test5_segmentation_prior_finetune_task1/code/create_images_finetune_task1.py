...
def instantiate_unconditioned_models(
    ...
    networks_config = {
        "diffusion_unet_def": {
            ...
            "spatial_dims": 3,
            "in_channels": 4 + dm_seg_channels,  # 4 for the latent and dm_seg_channels for the segmentation conditioning (concatenated)
            "out_channels": 4,
            "num_res_blocks": 2,
            "num_channels": [
                64,
...
def validation(
    ...
    print("src_modality_idx", modality_idx.item(), "src_resolution_idx", src_resolution_idx.item() if use_controlnet else None, "tar_resolution_idx", tar_resolution_idx.item())
...
def evaluate_task1(
    ...
    for i, row in df_val_task1.iterrows():
        ...
        print(f"Processing subject {row['sid']} with modality {modality} and resolution {resolution}T")
        
        if resolution in [0.1, 1.5]:  # float naming problem
            use_controlnet = False
            # continue
            tar_resolution_idx = 2  # = 7T
            
        else:
            use_controlnet = True
            tar_resolution_idx = 0
        ...
        src_latent_mask = load_segmentation(src_latent_mask_path, nb_classes=dm_seg_channels).unsqueeze(0)  # load the latent mask as float32 and add batch dimension
...
if __name__ == "__main__":
    ...
    dm_chk_dict = {
        "T1W": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test5_segmentation_prior/training/models/all_357t/segconcatenated/finetuning/test1_T1W_7T/check_points/model_6500_best.pt",
        "T2W": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test5_segmentation_prior/training/models/all_357t/segconcatenated/finetuning/test1_T2W_7T/check_points/model_8000_best.pt",
        "T2FLAIR": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test5_segmentation_prior/training/models/all_357t/segconcatenated/finetuning/test1_T2FLAIR_7T/check_points/model_6000_best.pt",
    }

    cnet_chk_dict = {
        "T1W": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test5_segmentation_prior/training/models/all_357t/segconcatenated/finetuning/test1_T1W_7T_controlnet/check_points/model_62750_best.pt",
        "T2W": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test5_segmentation_prior/training/models/all_357t/segconcatenated/finetuning/test1_T2W_7T_controlnet/check_points/model_64500_best.pt",
        "T2FLAIR": "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/experiments/test5_segmentation_prior/training/models/all_357t/segconcatenated/finetuning/test1_T2FLAIR_7T_controlnet/check_points/model_59500_best.pt",
    }
    ...
    for modality in ["T1W", "T2W", "T2FLAIR"]:
        ...
        # df_val = pd.read_csv("/home/agustin/phd/miccai/miccai_2026/mri_x_fields/data/csv/train_data.csv")
        # df_val = df_val[df_val["split"] == "val"]
        # output_path = "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/evaluation/test5_segmentation_prior_finetune_task1/results/train"


        output_path = "/home/agustin/phd/miccai/miccai_2026/mri_x_fields/evaluation/test5_segmentation_prior_finetune_task1/results/val"
        ...
        dm_chk_path = dm_chk_dict[modality]
        dm_chk_number = dm_chk_path.split("/")[-1].split("_")[1]  # get the checkpoint number from the path
        ...
        if not use_controlnet:
            ...
            controlnet_chk_path = cnet_chk_dict[modality]
            cnet_chk_number = controlnet_chk_path.split("/")[-1].split("_")[1]  # get the checkpoint number from the path
            # output_path = os.path.join(output_path, f"controlnet", f"chk_{dm_chk_number}_cnchk_{cnet_chk_number}_steps_{n_inference_steps}")
            output_path = os.path.join(output_path, f"controlnet")
        ...
        models_dict = instantiate_unconditioned_models(
            ...
            dm_num_resolutions=len(used_resolutions)- dm_tar_resolution_offset,  # because of the offset that we apply to the target resolution index
            dm_noise_scheduler_type="rflow",
        ...
        _df_val = df_val[df_val["modality"].isin([modality])].copy()    
