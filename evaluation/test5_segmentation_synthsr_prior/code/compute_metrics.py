...
def create_df_task3(pred_path_name, modalitites, resolutions):
    data = []
    for modality in modalitites:
        for src_resolution in resolutions:
            for tar_resolution in resolutions:
                if src_resolution == tar_resolution:
                    continue
                pred_path = os.path.join(pred_path_name, modality, f"{src_resolution}T_to_{tar_resolution}T", "pred")
                pred_files = glob.glob(os.path.join(pred_path, "*.nii.gz"))
                for pred_file in pred_files:
                    iid = os.path.basename(pred_file).replace(".nii.gz", "")
                    sid = "S" + iid.split("_")[-1]
...
def evaluate_row(args, test_segmentation_prior=False, crop_z=False, evaluate_seg=True):
    i, row = args
...
def evaluate_task_fast(df_original, pred_path_name, task, test_segmentation_prior=False, crop_z=False):
    ...
    if task == 3:
        pred_path_name = os.path.join(pred_path_name, "task3")

        modalitites = ["T1W", "T2W", "T2FLAIR"]
        resolutions = [0.1, 1.5, 3, 5, 7]
        df_task = create_df_task3(pred_path_name, modalitites, resolutions)
        
        # remove resolution col from df_original to save the source resolution and not always the target (7)
        merged_df = pd.merge(df_original, df_task[["iid", "src_resolution", "tar_resolution", "pred_img_path"]], on=["iid"], how="inner")
        ...
        df_original_src = df_original_src.rename(columns=rename)
        merged_df = pd.merge(merged_df, df_original_src, on=["sid", "modality", "src_resolution"], how="inner")
...
if __name__ == "__main__":
    ...
    # evaluate_task_fast(df_val, output_path, task=2, test_segmentation_prior=True, crop_z=crop_z)
    evaluate_task_fast(df_val, output_path, task=3, test_segmentation_prior=False, crop_z=crop_z)
