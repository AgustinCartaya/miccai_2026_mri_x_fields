    ...
    with torch.no_grad(), torch.amp.autocast("cuda"):
        ...
        for i, (t, next_t) in enumerate(progress_bar):
            ...
            if not isinstance(noise_scheduler, RFlowScheduler):
                latents, x0 = noise_scheduler.step(model_output, t, latents)
            else:
                latents, x0 = noise_scheduler.step(model_output, t, latents, next_t)  # type: ignore
...
# def load_pondered_segmentation(used_modalities, segmentation_npy_path, current_modality): 
#     seg_list = []
...
#             seg_list.append(load_segmentation(modified_path))
...
def evaluate_task1(
    ...
    for i, row in df_val_task1.iterrows():
        ...
        src_latent_mask_path = row[f"latent_seg_supersynth_merged_{dm_seg_channels}_path"]
        src_latent_synthsr_path = row[f"latent_synthsr_path"]

        # load always T1W seg and synthsr for 0.1 tesla T2w and flair
        if resolution == 0.1 and modality in ["T2W", "T2FLAIR"]:
        ...
        if use_synthsr:
            src_latent_synthsr = torch.from_numpy(np.load(src_latent_synthsr_path)).float().unsqueeze(0)  # load the latent synthsr as float32 and add batch dimension
        else:
            src_latent_synthsr = None
        ...
        synthetic_image = validation(
            ...
            src_latent_synthsr=src_latent_synthsr,
...
def evaluate_task3(
    ...
    for i, row in df_val_task3.iterrows():
        ...
        for tar_resolution_idx, tar_resolution in zip(
            ...
            bar.set_description(f"Generating synthetic images for Task 3 - sid: {row['sid']} Modality {modality} = {modality_idx} Resolution {src_resolution}T = {src_resolution_idx} -> {tar_resolution_idx}")
