...
class SABlock(nn.Module):
    ...
    def __init__(
        ...
        """
        # print("instantiating self attention block")
        super().__init__()
    ...
    def forward(self, x, attn_mask: Optional[torch.Tensor] = None):
        # print(f"in selfattention.py, x.shape: {x.shape}")
        ...
        # print(f"in selfattention.py step 0.1: x.min: {x.min()}, x.max: {x.max()}")

        # print(f"in selfattention.py step 0.2: q.min: {q.min()}, q.max: {q.max()}")
        # print(f"in selfattention.py step 0.3: k.min: {k.min()}, k.max: {k.max()}")
        # print(f"in selfattention.py step 0.4: v.min: {v.min()}, v.max: {v.max()}")
        # print(f"in selfattention.py step 0.5: attn_mask: {attn_mask}")
        ...
        if self.use_flash_attention:
            ...
            # print(f"in selfattention.py flash att step 1: x.min: {x.min()}, x.max: {x.max()}")
            # print(f"in selfattention.py flash att step 2: dropout_p: {self.dropout_rate}")
            # print(f"in selfattention.py flash att step 2: is_causal: {self.causal}")


        else:
            att_mat = torch.einsum("blxd,blyd->blxy", q, k) * self.scale
            # print(f"in selfattention.py, uniques in q: {torch.unique(q)}")
            # print(f"in selfattention.py, uniques in k: {torch.unique(k)}")
            # print(f"in selfattention.py, step 1: scale: {self.scale}")
            # print(f"in selfattention.py, step 1: att_mat.min: {att_mat.min()}, att_mat.max: {att_mat.max()}")
            ...
            # print(f"in selfattention.py, step 2: att_mat.min: {att_mat.min()}, att_mat.max: {att_mat.max()}")
            ...
            # print(f"in selfattention.py, step 3: att_mat.min: {att_mat.min()}, att_mat.max: {att_mat.max()}")

            x = torch.einsum("bhxy,bhyd->bhxd", att_mat, v)
            # print(f"in selfattention.py, step 4: x.min: {x.min()}, x.max: {x.max()}")
